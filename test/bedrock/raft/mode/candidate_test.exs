  ...
  describe "append_entries_ack_received/6" do
    ...
    test "becomes follower when term is higher", %{candidate: candidate} do
      assert :become_follower =
               Candidate.append_entries_ack_received(candidate, 2, true, {1, 1}, {1, 1}, :peer_1)
    end

    test "ignores when term is lower", %{candidate: candidate} do
      assert {:ok, _} =
               Candidate.append_entries_ack_received(candidate, 0, true, {1, 1}, {1, 1}, :peer_1)
    end

    test "ignores when term is equal", %{candidate: candidate} do
      assert {:ok, ^candidate} =
               Candidate.append_entries_ack_received(candidate, 1, false, {1, 1}, {1, 1}, :peer_1)
    end
  end
